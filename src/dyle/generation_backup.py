...
def generate_beam(model, tokenizer, lef_sentence, spans, beam_size = 3, 
    ...
    with torch.inference_mode():
        for i in range(lef-1, rig-1):
            # print(1)
            logits, _ = model(input_tokens, input_length, context, input_span)
            # print(2)
            logits = torch.cat((torch.zeros(logits.shape[0], len(lef_tokens), logits.shape[2]).cuda(), logits), dim=1)
